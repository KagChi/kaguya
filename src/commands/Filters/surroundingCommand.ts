...
@CommandConf({ 
    name: "surrounding",
    aliases: [],
    description: "surrounding filter",
    usage: "",
    cooldown: 3,
    ownerOnly: false
})
export default class surroundingCommand extends Command {
    public async exec(msg: Message, args: string[]) {
       ...
       const message = await msg.channel.send(`<a:emoji_28:668379222892347402> | Setting surrounding filter to \`${statusFilters ? "on" : "off"}\`. This may take a few seconds...`);
           const embed = this.client.util.embed()
           .setDescription(`<a:yes:739409625090228275> | surrounding filter set to \`${statusFilters ? "on" : "off"}\` `)
           .setColor(this.client.util.color);
            await delay(3500);
