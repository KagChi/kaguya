...
export default class vaporwaveCommand extends Command {
    public async exec(msg: Message, args: string[]) {
        ...
        let statusFilters = serverQueue.filters.vaporwave
        statusFilters = !statusFilters
       this.client.musicManager.setFilters(msg, {
           vaporwave: statusFilters
       })
       const message = await msg.channel.send(`<a:emoji_28:668379222892347402> | Setting vaporwave filter to \`${statusFilters ? "on" : "off"}\`. This may take a few seconds...`);
           const embed = this.client.util.embed()
           .setDescription(`<a:yes:739409625090228275> | vaporwave filter set to \`${statusFilters ? "on" : "off"}\` `)
           .setColor(this.client.util.color);
            await delay(3500);
