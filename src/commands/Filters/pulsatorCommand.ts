...
@CommandConf({ 
    name: "pulsator",
    aliases: [],
    description: "pulsator filter",
    usage: "",
    cooldown: 3,
    ownerOnly: false
})
export default class pulsatorCommand extends Command {
    public async exec(msg: Message, args: string[]) {
        ...
        let statusFilters = serverQueue.filters.pulsator
        statusFilters = !statusFilters
       this.client.musicManager.setFilters(msg, {
           pulsator: statusFilters
       })
       const message = await msg.channel.send(`<a:emoji_28:668379222892347402> | Setting pulsator filter to \`${statusFilters ? "on" : "off"}\`. This may take a few seconds...`);
           const embed = this.client.util.embed()
           .setDescription(`<a:yes:739409625090228275> | pulsator filter set to \`${statusFilters ? "on" : "off"}\` `)
           .setColor(this.client.util.color);
            await delay(3500);
