...
export default class tremoloCommand extends Command {
    public async exec(msg: Message, args: string[]) {
        ...
        let statusFilters = serverQueue.filters.tremolo
        statusFilters = !statusFilters
       this.client.musicManager.setFilters(msg, {
           tremolo: statusFilters
       })
       const message = await msg.channel.send(`<a:emoji_28:668379222892347402> | Setting tremolo filter to \`${statusFilters ? "on" : "off"}\`. This may take a few seconds...`);
           const embed = this.client.util.embed()
           .setDescription(`<a:yes:739409625090228275> | tremolo filter set to \`${statusFilters ? "on" : "off"}\` `)
           .setColor(this.client.util.color);
            await delay(3500);
